...
def create_chat_completion(task_type, prompt):
    ...
    response = neuralapi.ChatCompletion.create(
        ...
        functions=[config['function_info']],
        function_call={"name": config['function_info']['name']},
    )
...
def get_concept_title(text):
    ...
    response = create_chat_completion('title', prompt)
...
def get_concept_titles(prompt):
    response = create_chat_completion('concept_titles', prompt)
...
def get_concept_content(concept_title, source_context):
    ...
    response = create_chat_completion('concept_content', prompt)
    concept_content = json.loads(response.choices[0]["message"]["function_call"]["arguments"]).get("concept_content", False)
    concept_tags = json.loads(response.choices[0]["message"]["function_call"]["arguments"]).get("concept_tags", False)
    return concept_content, concept_tags

def get_linked_concept_content(concepts, source_context):
    concepts_info = "\n".join([f"title: {concept['title']}, tags: {concept['tags']}" for concept in concepts])
    ...
    response = create_chat_completion('linked_concept', prompt)
    concept_content = json.loads(response.choices[0]["message"]["function_call"]["arguments"]).get("concept_content", False)
    concept_tags = json.loads(response.choices[0]["message"]["function_call"]["arguments"]).get("concept_tags", False)
    return concept_content, concept_tags
